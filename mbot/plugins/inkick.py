...
@app.on_message(
    filters.incoming & ~filters.private & filters.command(["inkick"])
)
async def inkick(bot, message):
...
@app.on_message(
    filters.incoming & ~filters.private & filters.command(["uname"])
)
async def uname(bot, message):
    user = await bot.get_chat_member(message.chat.id, message.from_user.id)
    if user.status.value in ("administrator", "owner"):
        sent_message = await message.reply_text(
            "🚮**Sedang membersihkan user, mungkin butuh waktu beberapa saat...**"
...
async def rm_delacc(bot, message):
    user = await bot.get_chat_member(message.chat.id, message.from_user.id)
    if user.status.value in ("administrator", "owner"):
        sent_message = await message.reply_text(
            "🚮**Sedang membersihkan user, mungkin butuh waktu beberapa saat...**"
